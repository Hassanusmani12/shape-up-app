  ...
  return (
    <div className="w-100">
            ...
            {currentExercises.map((exercise, index) => (
              <Col key={exercise.id || index}>
                ...
                >
                  ...
                  <Card.Body style={{ background: 'var(--bg-card)' }}>
                    <Card.Title
      ...
      <Modal show={showModal} onHide={handleCloseModal} size="lg" centered>
        {modalExercise && (
          <>
                    <Modal.Header closeButton style={{ background: 'var(--bg-elevated)', borderBottom: '1px solid var(--border-subtle)' }}>
              <Modal.Title className="text-capitalize fw-bold text-adaptive-head">
                {modalExercise.name}
              </Modal.Title>
            </Modal.Header>
            <Modal.Body style={{ background: 'var(--bg-elevated)' }}>
              <Row>
                {/* Left Side: Image */}
                <Col md={5} className="mb-3 mb-md-0">
                  <div className="border rounded p-2 d-flex align-items-center justify-content-center" style={{ minHeight: "250px", background: 'var(--bg-card)', borderColor: 'var(--border-subtle)' }}>
                    <img 
            ...
            <Modal.Footer style={{ background: 'var(--bg-elevated)', borderTop: '1px solid var(--border-subtle)' }}>
              <Button variant="secondary" onClick={handleCloseModal}>
                Close
              </Button>
              <Button variant="success" onClick={handleAddToLog} style={{ background: 'var(--neon-green)', color: '#000', border: 'none' }}>
                Add to Workout Log
              </Button>
            </Modal.Footer>
          </>
...
export default React.memo(ExerciseDB);